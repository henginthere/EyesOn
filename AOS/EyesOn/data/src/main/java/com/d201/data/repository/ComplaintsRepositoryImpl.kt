...
class ComplaintsRepositoryImpl @Inject constructor(
    ...
    override fun insertComp(
        complaintsRequest: MultipartBody.Part,
        imageFile: MultipartBody.Part
    ): Flow<ResultType<BaseResponse<Boolean>>> = flow {
    ...
    override fun selectComplaintsBySeq(seq: Long): Flow<ResultType<BaseResponse<Complaints>>> =
        flow {
            emit(ResultType.Loading)
    ...
    override fun returnComplaints(complaints: Complaints): Flow<ResultType<BaseResponse<Void>>> =
        flow {
            emit(ResultType.Loading)
            complaintsRemoteDataSource.returnComplaints(complaints.mapperToComplaintsRequest())
                .collect {
                    emit(
                        ResultType.Success(
                            BaseResponse(
    ...
    override fun submitComplaints(complaints: Complaints): Flow<ResultType<BaseResponse<Void>>> =
        flow {
            emit(ResultType.Loading)
            complaintsRemoteDataSource.submitComplaints(complaints.mapperToComplaintsRequest())
                .collect {
                    emit(
                        ResultType.Success(
                            BaseResponse(
    ...
    override fun completeComplaints(complaints: Complaints): Flow<ResultType<BaseResponse<Void>>> =
        flow {
            emit(ResultType.Loading)
            complaintsRemoteDataSource.completeComplaints(complaints.mapperToComplaintsRequest())
                .collect {
                    emit(
                        ResultType.Success(
                            BaseResponse(
